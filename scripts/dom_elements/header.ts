...
export class header extends div {
  ...
  private level: DOMElement = $.span().inner("1");
  private btn_left: DOMElement = $.div().inner("&#x25c0;&#xfe0e;");
  private btn_right: DOMElement = $.div().inner("&#x25b6;&#xfe0e;");
  private dots: DOMElement = $.div().stl(
    `display:inline-block;vertical-align:middle`
  );
  private prefix: DOMElement = $.div().stl(
    `display:inline-block;vertical-align:middle;text-align:right`
  );
  private suffix: DOMElement = $.div().stl(
  ...
  constructor(_cb: (n: number) => void) {
    ...
    this.stl(
      `display:block;position:relative;width:${s.width}px;height:${
        h.height + t.height
      }px;color:${h.color};background-color:${
        h.background
    ...
    $("body").add(
      (this.fx_banner = $.div()
        .att("id:top_banner")
        .stl(
          `position:fixed;z-index:1;left:0px;top:0px;width:${
            s.width
          }px;height:${
            h.height + t.height
          }px;text-align:center;visibility:hidden`
        )
        .add(
          $.div()
            // *** Bandeau de titre ***
            ...
            .add(
              ...
              $.div()
                .stl(
                  `display:table-cell;max-width:${
                    s.width - (isMB ? t.height : 2 * t.height)
                  }px;height:${
                    t.height
                  }px;vertical-align:middle;font-family:sans-serif;font-size:${
                    t.fontsize
                  }px;color:${
                    t.color
                  };font-weight:bold;text-align:left;padding-left:10px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;`
                )
                .inner(t.text)
            )
            .add(
              ...
              isMB
                ...
                : $.div()
                    ...
                    .add(
                      $.create("a")
                        .att(
                          `href:https://files.dgpad.net/qrcode/?url=${location.href};target:_blank`
                        )
                        .add(
                          $.create("img")
        ...
        .add(
          (this.table = $.div()
            .stl(
              `display:table;width:${s.width}px;height:${h.height}px;color:${h.color};background-color:${h.background};user-select:none;-webkit-user-select:none`
            )
            .add(
              $.div()
                ...
                .add(
                  $.div()
                    .stl(
                      `display:table-cell;vertical-align:middle;text-align:center;font-family:sans-serif;`
                    )
                    .add(
                      $.div()
                        .add(
                          this.prefix
                            .inner(h.prefix.text)
                            .stl(
                              `font-size:${h.prefix.fontsize}px;width:${h.prefix.width}px;padding-right:${h.prefix.padding}px`
                            )
                        )
                        .add(this.dots)
                        .add(
                          this.suffix.stl(
                            `padding-left:${h.suffix.padding}px;width:${h.suffix.width}px;font-family:sans-serif;font-size:${h.suffix.fontsize}px`
                          )
                        )
                    )
                )
    ...
    if (this.nbLevels > 1) {
      ...
      this.table.add(
        $.div()
          ...
          .add(
            (this.row2 = $.div().stl(
              `display:table-cell;height:${l.height}px;vertical-align:top`
            ))
          )
      );
      for (var i = 0; i < this.nbLevels; i++) {
        this.row2.add(
          $.div()
            .att(`id:level_block`)
            .stl(
              `position: relative;display:inline-block;margin-left:5px;margin-right:5px;margin-top:10px;vertical-align:top`
            )
            .add(
              $.div()
                .att("id:level_button")
                .stl(
                  `display:block;font-size:${b.fontsize}px;font-family:sans-serif;background:${b.background};color:${b.color};border-radius: 8px;border: 2px solid ${b.border_color};box-sizing: border-box;cursor: pointer;font-weight: 400;outline: none;padding: 5px 12px;user-select: none;-webkit-user-select: none`
                )
                .inner(b.text + (i + 1))
                .hover(
                  `background:${b.hover_background};color:${b.hover_color}`
                )
    ...
    setTimeout(() => {
      $("#top_banner").stl(
        `top:${-$("#top_banner").height()}px;visibility:visible;`
      );
      setTimeout(() => {
  ...
  private change_level(e: PointerEvent) {
    const l = window.$SETTINGS.header.levels;
    const b = l.button;
    const rank = (<Element>e.currentTarget).dom_element.getValue().rank;
    ...
    if (!btns[rank].getValue().selected) {
      ...
      for (let k = 0; k < btns.length; k++) {
        ...
        if (btns[k].getValue().selected) old_rank = k;
        btns[k].getValue().selected = false;
      }
      this.display_score(old_rank);
      btns[rank].stl(
        `background:${b.select_background};color:${b.select_color}`
      );
    ...
    for (var i = 0; i < n; i++) {
      this.dots.add(
        $.div()
          .att(`id:virgin_dot`)
          .stl(
            `box-sizing: border-box;display:inline-block;margin-left:${
              h.dots.gap / 2
            }px;margin-right:${h.dots.gap / 2}px;background:${
              h.dots.virgin_background
            };vertical-align:middle;border-radius:100%;width:${w}px;height:${w}px;border:2px solid ${
              h.dots.virgin_color
            }`
          )
      );
  ...
  public no_more_dots = function () {
    let dots = this.dots.findAll("#virgin_dot");
    return dots.length === 0;
  };

  public mark_dot(b: boolean): boolean {
    const h = window.$SETTINGS.header;
    if (b) {
    ...
    dots[0]
      ...
      .stl(
        `background:${
          b ? h.dots.win_background : h.dots.fail_background
        };border:2px solid ${b ? h.dots.win_color : h.dots.fail_color}`
      );
