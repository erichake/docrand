import { $, DOMElement } from "../../scripts/dom_design.js";
...
export class MOBILE_KEYBOARD {
  ...
  constructor() {
    if (this.isMobileDevice()) {
      ...
      this.keyboard = new window.SimpleKeyboard.default({
        onChange: (input: any) => me.onChange(input),
        onKeyPress: (button: any) => me.onKeyPress(button),
        layout: {
          default: ["- 1 2 3 4 5 {bksp}", ", 6 7 8 9 0 {enter}"],
        },
        display: {
          "{bksp}":
            "<img style='width:48px;margin-top:10px' src='./ressources/images/bksp.svg'/>",
  ...
  isMobileDevice() {
    if (window.hasOwnProperty("$MOBILE_MODE")) return window.$MOBILE_MODE;
  ...
  actualise(infos: { options?: any; exit_proc?: any; elt: any }) {
    if (this.isMobileDevice()) {
      var me = this;
      me.show();
      ...
      for (var i = 0; i < me.INPS.length; i++) {
        if (me.INPS[i].wrapper().id() !== "mobile_keyboard_input") {
          ...
          div.getValue().rank = i;
          div.down(function (e) {
            me.select((<Element>e.target).dom_element.getValue().rank);
  ...
  onKeyPress(button: any) {
    ...
    switch (button) {
      case "{next}":
        ...
        break;
      case "{enter}":
        if (me.selection < me.DIVS.length - 1) me.select(me.selection + 1);
        else {
          me.selection = 0;
          me.exit_proc();
        }
      ...
      case "{bksp}":
        break;
      default:
        button.replace(/^{(.*)}$/, (_a: any, _m: any) => {
          const inp = me.INPS[me.selection].dom() as HTMLInputElement;
