...
export class stage extends div {
    constructor() {
        ...
        this.$c = new content();
        this.$f = new footer(this.getKBD.bind(this));
        this.current_level = -1;
        this.KBD = new MOBILE_KEYBOARD();
        this.is_cache = false;
        this.time = 0;
        this.interval = 0;
        ...
        let d = $.div()
            ...
            .stl(`position:absolute;display:none;left:0px;top:0px;width:100%;height:100%;min-height:300px`))
            .add($.div()
            .att(`id:chrono_progress_bar`)
            .stl(`position:absolute;left:0px;top:0px;height:5px;width:0px;background:#ff8c1a`))
            .add($.div()
            .att(`id:chrono_start_cache`)
            .stl(`position:absolute;display:none;left:0px;top:0px;width:100%;height:100%;min-height:300px;background:${s.chrono.cache.background}`)
            .add($.create("a")
            .att(`id:chrono_start_btn`)
            .stl(`position:absolute;font-family:sans-serif;font-size:${s.chrono.cache.fontsize2}px;padding:8px 25px;border-radius:5px;border:0;cursor:pointer;text-align:center;background:#1eaad0;color:white;font-weight:bold;bottom:30px;left:50%;transform:translate(-50%,0)`)
            .inner("Démarrer !")
            ...
            .stl(`position:absolute;left:50%;top:0px;width:90%;margin-top:40px;transform:translate(-50%,0);text-align:justify;font-family:sans-serif;font-size:${s.chrono.cache.fontsize1}px;`)));
        let snp = $.div()
        ...
        if (window.$START_LEVEL > 0 && window.$START_LEVEL <= btns.length) {
            ...
            if (L !== 0) {
                ...
                btns[L].stl(`background:${prf.select_background};color:${prf.select_color}`);
            }
        }
        else {
            this.setLevel(1);
    ...
    makeSnapshot() {
        ...
        for (var i = 0; i < inps_dest.length; i++) {
            let i_dest = inps_dest[i].dom();
            let i_srce = inps_srce[i].dom();
    ...
    swipeLeft() {
        ...
        $("#swipe_wrapper").stl(`left:0px;transition:${t}s ease-in-out;transform:translate(${-ww}px,${0}px)`);
        setTimeout(() => {
        ...
        if (this.KBD.isMobileDevice()) {
            ...
            if ($("body").findAll("landscape_message_cache").length === 0) {
                $("body").add($.div()
                    ...
                    .stl(`position:fixed;z-index:1000;left:0px;top:0px;width:0;height:0;background-color:#00000000;display:none;transition:all ${ttime}s ease-in-out`)
                    .down(function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                })
                    .inner(``)
                    .add($.div()
                    .stl(`position:absolute;width:80%;display:block;left:50%;top:50%;transform:translate(-50%,-50%)`)
                    .add($.create("img")
                    .att(`src:./ressources/images/lanscape_message.svg`)
                    .stl(`display:block;max-width:40%;margin:0 auto`))
                    .add($.div()
                    .stl(`display:block;color:#AAA;margin:0 auto;margin-top:20px;font-family:sans-serif;font-size:28px;font-weight:lighter;text-align:center`)
            ...
            if (this.is_cache) {
                ...
                $("#landscape_message_cache").stl(`display:block;width:${ww}px;height:${wh}px`);
    ...
    resize() {
        ...
        let stls = `left:${(dw - ww) / 2}px;top:0px;transform-origin:top left;transform:scale(${this.scale})`;
        this.stl(stls);
        $(`#top_banner`).stl(stls);
        if (this.KBD.isMobileDevice()) {
            this.KBD.dom.stl(`left:${(dw - ww) / 2}px;transform-origin:bottom left;transform:scale(${this.scale})`);
            $(`#student_answer_footer`).stl(`margin-bottom:${this.KBD.dom.height() / this.scale}px`);
        }
    }
    getQUE(_m) {
    ...
    setLevel(_n) {
        let new_level = this.current_level !== _n;
        ...
        if (new_level) {
            ...
            if (this.QUE.getCHRONO() > 0) {
                $(`#chrono_start_txt`).inner(`<img src="./ressources/images/chrono.png" style="max-width:120px;float:right;padding-left:10px"/>Ce niveau de jeu est chronométré. Tu as ${this.QUE.getCHRONO()} secondes pour répondre à chacune des ${this.QUE.getNB()} questions de ce niveau.<br><br>Clique sur le bouton ci-dessous pour commencer :`);
    ...
    countdown() {
        ...
        let interval = function () {
            ...
            if (t < $COUNTDOWN_START) {
                $("#countdown_cache").stl(`top:${$COUNTDOWN_SIZE * (t / $COUNTDOWN_START)}px`);
                if ($("#countdown_circle").dom().innerText !==
                    "" + count) {
                    $("#chrono_countdown").stl(`background-color:${getRandomLightColor()}`);
        ...
        $("#chrono_countdown").add($.div().att("id:countdown_back").add($.div().att("id:countdown_cache")));
        $("#chrono_countdown").add($.div()
    ...
    endLevel() {
        ...
        if ($("body").findAll("dom_design_dialog_cache").length === 0) {
            $("body").add($.div()
                .stl(`position:absolute;z-index:1000;left:0px;top:0px;width:100%;height:100%;background-color:#00000000;display:none;transition:all ${ttime}s ease-in-out`)
        ...
        $("#dom_design_dialog_cache")
            ...
            .show()
            .add($.create("div")
            .stl(`position:absolute;z-index:1001;left:0%;top:0%;background-color:#F0F0F0;width:0px;height:0px;border-radius:0px;display:inline-block;transform:translate(-50%,-50%);font-family: sans-serif;font-size:0px;color:black;padding:0px;pointer-events:all;overflow:hidden;transition:all ${ttime}s ease-in-out`)
            .att("id", "dom_design_dialog_box")
            .add($.div()
            .stl(`position:absolute;left:0px;top:10px;width:100%;text-align:center;font-size:${p.dialog.fontsize1}px;font-weight:bold;color:${p.dialog.color1}`)
            .inner(p.dialog.text1))
            .add($.div()
            .stl(`position:absolute;left:0px;top:60px;width:100%;text-align:center;font-size:${p.dialog.fontsize2}px;color:${p.dialog.color2}`)
            .inner(`${p.dialog.text2}${this.$h.get_score()} / ${this.QUE.getNB()}`))
            .add($.create("a")
            .att(`id:dom_design_dialog_btn`)
            .stl(`position:absolute;width:80px;height:30px;right:10px;bottom:10px;line-height:30px;border-radius:5px;border:0;cursor:pointer;font-size:16px;text-align:center;background:#8cd4f5;color:white;font-weight:bold`)
            .inner("Ok")
            .up(() => {
            clearInterval(this.interval);
            $("#dom_design_dialog_box").stl(`width:0px;height:0px;border-radius:0px;padding:0px;font-size:0px;top:0%;left:100%`);
            ...
            .enter(() => $("#dom_design_dialog_btn").stl(`background:#1eaad0`))
            .out(() => $("#dom_design_dialog_btn").stl(`background:#8cd4f5`))));
        setTimeout(() => {
            $("#dom_design_dialog_cache").stl(`background-color:#000000AA`);
            $("#dom_design_dialog_box").stl(`width:300px;height:120px;border-radius:8px;padding:20px;font-size:20px;top:50%;left:50%`);
        }, 1);
    }
}
